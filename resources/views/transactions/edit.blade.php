...
<div class="min-h-screen py-12 bg-gradient-to-b from-pink-50 via-pink-100 to-pink-200 flex items-center justify-center">
    <div class="w-full max-w-3xl px-6">
        ...
        <div class="bg-white rounded-3xl shadow-2xl p-10 border border-pink-200">
            ...
            <form action="{{ route('transactions.update', $transaction->id ?? '') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="number" name="amount" value="{{ old('amount', $transaction->amount ?? 0) }}" required min="0" step="1"
                               class="w-full rounded-2xl border-2 border-pink-400 px-5 py-4 placeholder-pink-300 focus:outline-none focus:ring-2 focus:ring-pink-400 focus:border-pink-500 transition duration-300 text-pink-800 font-semibold">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div id="image-preview" class="mt-4 {{ !empty($transaction->cover) ? '' : 'hidden' }}">
                        <img id="preview-img" src="{{ !empty($transaction->cover) ? Storage::url($transaction->cover) : '' }}" alt="Preview" class="max-w-xs rounded-2xl border-2 border-pink-400 shadow-xl">
                    </div>
                </div>
