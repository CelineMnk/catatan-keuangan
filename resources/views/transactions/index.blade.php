...
<div class="py-10 theme-bg min-h-screen">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="flex items-center justify-end mb-6 space-x-3">
            <a href="{{ route('transactions.stats') }}" class="btn-fancy btn-purple">
                <span class="mr-2">📊</span> Statistik
            </a>
            <a href="{{ route('transactions.create') }}" class="btn-fancy btn-pink">
        ...
        <div class="filter-box mb-8">
            <form method="GET" action="{{ route('transactions.index') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div>
                    ...
                    <input id="search" name="search" type="text" value="{{ request('search') }}" placeholder="Judul atau deskripsi...">
                </div>
            ...
            <table class="min-w-full divide-y divide-pink-200">
                ...
                <tbody class="divide-y divide-pink-100">
                    @forelse($transactions as $transaction)
                        <tr>
                            <td class="px-6 py-4 text-gray-700">{{ $transaction->transaction_date ? date('d/m/Y', strtotime($transaction->transaction_date)) : '-' }}</td>
                            <td class="px-6 py-4 text-gray-800">
                                <div class="flex items-center space-x-3">
                                    @if($transaction->cover)
                                        {{-- 🔽 Perbaikan ukuran gambar --}}
                                        <img src="{{ Storage::url($transaction->cover) }}" class="transaction-img">
                                    @else
                                        <div class="h-10 w-10 rounded-full bg-pink-100 flex items-center justify-center text-xs text-pink-700">No img</div>
                                    @endif
                                    <div>
                                        <div class="font-bold text-pink-900">{{ $transaction->title }}</div>
                                        <div class="text-sm text-gray-600">{!! Str::limit(strip_tags($transaction->description), 60) !!}</div>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4 font-medium">
                                <a href="{{ route('transactions.edit', $transaction) }}" class="text-purple-600 hover:text-purple-800 font-bold mr-3">Edit</a>
                                <form action="{{ route('transactions.destroy', $transaction) }}" method="POST" class="inline" onsubmit="return false;">
                                    @csrf
                                    @method('DELETE')
